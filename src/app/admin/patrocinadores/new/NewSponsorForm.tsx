...
export function NewSponsorForm() {
  ...
  const [formData, setFormData] = useState({
    nome: '',
    link: '',
    nivel: 'bronze' as 'master' | 'ouro' | 'prata' | 'bronze',
    ativo: true
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createPatrocinador(formDataToSend)
      
      success('Patrocinador criado!', 'O novo patrocinador foi adicionado com sucesso.')
      router.push('/admin/patrocinadores')
    } catch (error) {
      console.error('Error creating patrocinador:', error)
      showError('Erro ao criar patrocinador', error instanceof Error ? error.message : 'Tente novamente mais tarde.')
    } finally {
      setLoading(false)
    }
  }
